...
create or replace view ANALYTICS_DEV.AUX.INT_CATEGORIES as (with relevant_fields as (
  select
    category_id
    , category_name
    , case 
...
comment if exists on view ANALYTICS_DEV.AUX.INT_CATEGORIES IS 'Intermediate table for categories.';
comment if exists on column ANALYTICS_DEV.AUX.INT_CATEGORIES.category_id IS 'A unique ID number for each category.';
