...
create or replace view ANALYTICS_DEV.AUX.INT_STORES as (with relevant_fields as (
  select
    store_id
    , county_id
    , store_name
...
comment if exists on view ANALYTICS_DEV.AUX.INT_STORES IS 'Intermediate table for stores.';
comment if exists on column ANALYTICS_DEV.AUX.INT_STORES.store_id IS 'A unique ID number for each store.';
