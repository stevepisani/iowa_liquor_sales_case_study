...
create or replace view ANALYTICS_DEV.AUX.INT_VENDORS as (with relevant_fields as (
  select
    vendor_id
    , vendor_name
    , dt as effective_dt
...
comment if exists on view ANALYTICS_DEV.AUX.INT_VENDORS IS 'Intermediate table for vendors.';
comment if exists on column ANALYTICS_DEV.AUX.INT_VENDORS.vendor_id IS 'A unique ID number for each vendor.';
