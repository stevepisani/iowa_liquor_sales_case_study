...
create or replace table ANALYTICS.MARTS.INVOICES as (select
  invoices.invoice_id
...
comment if exists on table ANALYTICS.MARTS.INVOICES IS 'A denormalized table of invoices with all relevant information.';
comment if exists on column ANALYTICS.MARTS.INVOICES.invoice_id IS 'A unique ID number for each invoice.';
